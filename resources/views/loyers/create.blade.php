    ...
    <div class="card loyer-card">
        <div class="card-header loyer-card-header d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="mb-0 small">Enregistrer un nouveau paiement pour un étudiant.</p>
            </div>
        ...
        <div class="card-body">
            <form action="{{ route('loyers.store') }}" method="POST" class="row g-3">
                @csrf
        ...
        <div class="col-md-3">
            ...
            <select name="mois" id="mois" class="form-select @error('mois') is-invalid @enderror" required>
                ...
                @foreach ($mois as $m)
                    <option value="{{ $m }}" @selected(old('mois') == $m)>{{ $m }}</option>
                @endforeach
            </select>
            @error('mois')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-md-3">
            ...
            <select name="annee" id="annee" class="form-select @error('annee') is-invalid @enderror" required>
                ...
                @foreach ($annees as $a)
                    <option value="{{ $a }}" @selected(old('annee') == $a)>{{ $a }}</option>
                @endforeach
            </select>
            @error('annee')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-md-6">
            ...
            <select name="statut" id="statut" class="form-select @error('statut') is-invalid @enderror" required>
                <option value="paye" @selected(old('statut') == 'paye')>Payé</option>
                <option value="non_paye" @selected(old('statut', 'non_paye') == 'non_paye')>Non payé</option>
            </select>
            @error('statut')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
